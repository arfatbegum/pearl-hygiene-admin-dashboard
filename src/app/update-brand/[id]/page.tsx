...
const UpdateBrand = ({ params }: any) => {
    ...
    const getProductById = (productId: any) => {
        const brandRef = ref(database, `brands/${productId}`);
        get(brandRef)
            .then((snapshot) => {
                if (snapshot.exists()) {
                    const brandData = snapshot.val();
                    setBrand(brandData);
                    setBrandId(productId);
                    setBrandDetails(brandData.details);
                    setBrandUrl(brandData.url);
                    setImageUrl(brandData.image);
                } else {
                    Swal.fire('Error', `No brand found with ID: ${productId}`, 'error');
                }
            })
    ...
    const handleSubmit = async (e: any) => {
        ...
        const brandsRef = ref(database, `brands/${id}`);
                    ...
                    <div className="p-6.5">
                        ...
                        <div className="mb-4.5">
                            ...
                            <Editor
                                ...
                                // @ts-ignore
                                value={brandDetails || ''}
                                onEditorChange={(content) => setBrand({ ...brand, brandDetails: content })}
                                init={{
                                    height: 300,
                                    menubar: false,
                                    plugins: [
